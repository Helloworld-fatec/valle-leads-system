// src/components/funilVendas/NegotiationCard.tsx
...
export default function NegotiationCard({ negotiation, color, isDragging = false }: Props) {
  ...
  // Importância mais recente
  const importanceHistory = negotiation.importance_history ?? [];
  const rawImportance = importanceHistory[importanceHistory.length - 1]?.importance ?? "morno";
  const imp = importanceConfig[rawImportance as keyof typeof importanceConfig];
  ...
  return (
    <>
      ...
      >
        ...
        {/* Footer: cor da coluna como indicador visual */}
        <div
