// src/components/dashboards/manager/ManagerIdleLeadsChart.tsx
// Leads da equipe SEM negociação aberta (snapshot):
...
interface ManagerIdleLeadsChartProps {
...
export default function ManagerIdleLeadsChart({ data, loading }: ManagerIdleLeadsChartProps) {
  const idle = data?.idleLeads ?? null;

  if (loading) {
    return (
      <div
        className="rounded-2xl p-5 shadow-sm border animate-pulse"
  ...
  if (!idle || idle.total === 0) {
    return (
      <div
        className="rounded-2xl p-5 shadow-sm border flex flex-col items-center justify-center min-h-[220px]"
        style={{ background: "#FFFFFF", borderColor: "#E5E7EB" }}
      >
        <p className="text-sm font-medium" style={{ color: "#16A34A" }}>
          Nenhum lead parado na equipe 🎉
        </p>
